...
import network.MySession; // bắt buộc có
...
public class BossManager implements Runnable {
    ...
    public void showListBoss(Player player) {
        ...
        try {
            ...
            if (session.version >= 214) { // thực hiện logic khi version >= 214         
}
   ...
   public void run() {
    ...
    public void sendBossStatusMenu(player.Player player) {
        try {
            StringBuilder sb = new StringBuilder();
            sb.append("Trạng thái Boss:\n");
            for (boss.Boss boss : this.BOSSES) {
                sb.append(boss.name)
                  .append(": ")
                  .append(boss.isDie() ? "Đã bị tiêu diệt" : "Còn sống")
                  .append("\n");
            }
            player.npcChat("Danh sách Boss", sb.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static class OtherBossManager {

        public OtherBossManager() {
        }
    }

    static class BrolyManager {
